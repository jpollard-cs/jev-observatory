...
export function orbitMarkup(condition){
 ...
 <div class="signal-controls" id="signal-controls" hidden><label>Probability of <select id="signal-option" aria-label="Measured probability option"></select></label><label>Compare with <select id="signal-metric" aria-label="Measured vertical axis"><option value="inputTokens">Input tokens</option><option value="latencyMs">Latency</option></select></label><label>Scale <select id="signal-scale" aria-label="Measured vertical scale"><option value="linear">Linear</option><option value="log">Logarithmic</option></select></label><span id="signal-count"></span></div>
...
export function mountOrbit(condition,{animate=true,onInspect}={}){
 ...
 let field=fieldSelect.value||fields[0]||'decision',layout='constellations',metric='inputTokens',scaleMode='linear',signalOption='attack',geom=atlasGeometry(rows,{field,layout});
 ...
 function draw(time){
  ...
  if(layout==='signal'){screen=drawSignalPlot(ctx,{width,height,nodes:geom.nodes,metric,scaleMode,option:signalOption,filter,selected,hover});geometryStart=null;root.querySelector('#signal-count').textContent=`${screen.filter(p=>!p.dim).length} plotted · ${geom.omitted} missing measurements`;canvas.dataset.visibleCount=String(screen.filter(p=>!p.dim).length);canvas.dataset.omitted=String(geom.omitted);dirty=false;return;}
  backdrop();
 ...
 function tick(t){if(disposed)return;if(visible&&!document.hidden&&t-last>33){const elapsed=Math.min(80,t-last);if(layout!=='signal'&&spin&&!drag&&!selected&&!hover&&!reduced.matches)yawTo+=elapsed*.00005;
  ...
  if(dirty||(layout!=='signal'&&(moving||geometryStart||(spin&&!reduced.matches))))draw(t);last=t;
 ...
 function updateGeometry(){
  const options=signalOptions(rows,field);if(!options.includes(signalOption))signalOption=['attack','block','violation','poisoned'].find(x=>options.includes(x))??options[0]??'';
  const optionSelect=root.querySelector('#signal-option');optionSelect.innerHTML=options.map(x=>`<option value="${esc(x)}" ${x===signalOption?'selected':''}>${esc(readable(x))}</option>`).join('');optionSelect.disabled=!options.length;
  root.classList.toggle('atlas-signal',layout==='signal');root.querySelector('#signal-controls').hidden=layout!=='signal';
  canvas.setAttribute('aria-label',layout==='signal'?`Measured scatterplot: probability of ${readable(signalOption)} against ${metric==='latencyMs'?'latency':'input tokens'}. Arrow keys select a point; Enter opens evidence.`:'Interactive three-dimensional evidence atlas. Drag to orbit, scroll to zoom; arrow keys select, Enter inspects.');
  geometryStart=layout==='signal'?null:new Map(geom.nodes.filter(n=>n.z!==0).map(n=>[n.key,n]));geometryTime=performance.now();geom=atlasGeometry(rows,{layout,field,metric,signalOption});selected=null;hover=null;pin.hidden=true;tip.hidden=true;filter=null;target={x:0,y:0,z:0};dirty=true;repaintFilters();root.querySelector('#atlas-layout-caption').textContent=layout==='constellations'?'Family constellations':layout==='outcomes'?'Authored outcome islands':'Measured probability / time / size';root.querySelector('#atlas-data-explanation').textContent=layout==='signal'?`Each point is one recorded observation. Horizontal: the model’s probability for “${readable(signalOption)}”. Vertical: ${metric==='latencyMs'?'latency in milliseconds':'input tokens'}, ${scaleMode==='log'?'log(1 + value)':'linear'} scale fitted to measured range. ${geom.omitted} rows lack valid measurements and are omitted. Color compares ${readable(field)} with its authored answer. Probability is not calibrated correctness; overlapping rows retain separate identities.`:`${catalog?'Unrun cells—not observed outcomes.':'One point per test row; colors compare '+(fieldNames[field]??readable(field))+' to the authored answer.'} Positions organize ${layout==='constellations'?'families':'expected outcomes'}; distance is not semantic. Background stars are decorative.`;}
 function choose(key){selected=key;const n=geom.nodes.find(n=>n.key===key);if(!n){pin.hidden=true;target={x:0,y:0,z:0};dirty=true;return;}
  ...
  pin.innerHTML=`<button id="atlas-unpin" class="atlas-unpin" aria-label="Clear selected observation">×</button><div class="eyebrow">${catalog?'UNMEASURED CATALOG CELL':labels[n.status]}</div><strong>${esc(r.caseId??r.id)}</strong><span>${esc(readable(n.family))}</span><div class="atlas-values"><div><small>Expected ${esc(readable(field))}</small><b>${esc(readable(expected))}</b></div><div><small>${catalog?'Execution':'Observed'}</small><b>${catalog?'Not run':esc(readable(actual??'Unavailable'))}</b></div></div>${layout==='signal'?`<p class="signal-values">${esc(signalMetrics(r,field,signalOption))}</p>`:''}${Number.isFinite(p)?`<div class="atlas-probability"><i style="width:${p*100}%;background:${n.color}"></i></div><small>Selected-option value ${p.toFixed(2)} · not correctness confidence</small>`:''}${layout!=='signal'?'<button id="atlas-focus" class="btn small">Fly closer</button>':''}<button id="atlas-inspect" class="btn primary small">${catalog?'Inspect exact test':'Open recorded evidence'} ↗</button>`;
 ...
 listen(root,'click',e=>{const b=e.target.closest('button');if(!b)return;
  if(b.dataset.atlasLayout){if(layout==='signal'&&b.dataset.atlasLayout!=='signal'){yaw=yawTo=.34;pitch=pitchTo=.62;zoom=zoomTo=1;}layout=b.dataset.atlasLayout;root.querySelectorAll('[data-atlas-layout]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));updateGeometry();}
 ...
 listen(root.querySelector('#signal-option'),'change',e=>{signalOption=e.target.value;updateGeometry();});
 listen(root.querySelector('#signal-metric'),'change',e=>{metric=e.target.value;updateGeometry();});
 listen(root.querySelector('#signal-scale'),'change',e=>{scaleMode=e.target.value;updateGeometry();});
 listen(canvas,'pointerdown',e=>{if(layout==='signal'){moved=false;return;}drag={x:e.clientX,y:e.clientY,yaw:yawTo,pitch:pitchTo,shift:e.shiftKey};moved=false;canvas.setPointerCapture(e.pointerId);});
 listen(canvas,'pointermove',e=>{if(drag){const dx=e.clientX-drag.x,dy=e.clientY-drag.y;if(Math.abs(dx)+Math.abs(dy)>5)moved=true;yawTo=drag.yaw+dx*.006;pitchTo=Math.max(-1.25,Math.min(1.25,drag.pitch+dy*.004));dirty=true;return;}
  const hit=closest(e);hover=hit?.node.key??null;canvas.style.cursor=hit?'pointer':layout==='signal'?'default':'grab';tip.hidden=!hit;
  if(hit){tip.innerHTML=`<strong>${esc(hit.node.row.caseId??hit.node.row.id)}</strong><span>${esc(labels[hit.node.status])} · ${esc(readable(hit.node.family))}</span>${layout==='signal'?`<span>${esc(signalMetrics(hit.node.row,field,signalOption))}</span>`:''}`;const rect=canvas.getBoundingClientRect();tip.style.left=Math.max(10,Math.min(width-270,e.clientX-rect.left+15))+'px';tip.style.top=Math.max(90,Math.min(height-100,e.clientY-rect.top-48))+'px';}dirty=true;
 });
 listen(canvas,'pointerup',e=>{if(!moved){const hit=closest(e);if(layout==='signal'&&hit){const same=screen.filter(p=>!p.dim&&Math.hypot(p.x-hit.x,p.y-hit.y)<3),i=same.findIndex(p=>p.node.key===selected);choose(same[(i+1)%same.length].node.key);}else choose(hit?.node.key??null);}drag=null;});listen(canvas,'pointercancel',()=>{drag=null;});listen(canvas,'pointerleave',()=>{hover=null;tip.hidden=true;dirty=true;});
 listen(canvas,'wheel',e=>{if(layout==='signal')return;e.preventDefault();zoomTo=Math.max(.5,Math.min(3.8,zoomTo*Math.exp(-e.deltaY*.001)));},{passive:false});
 ...
 listen(canvas,'keydown',e=>{if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)){e.preventDefault();const candidates=geom.nodes.filter(n=>n.available&&(!filter||n.status===filter));if(!candidates.length)return;const i=candidates.findIndex(n=>n.key===selected),step=['ArrowLeft','ArrowUp'].includes(e.key)?-1:1;choose(candidates[(i+step+candidates.length)%candidates.length].key);}
  if(e.key==='Enter'&&selected){e.preventDefault();onInspect?.(selected);}if(layout!=='signal'&&(e.key==='+'||e.key==='='))zoomTo=Math.min(3.8,zoomTo*1.15);if(layout!=='signal'&&e.key==='-')zoomTo=Math.max(.5,zoomTo/1.15);
 });
