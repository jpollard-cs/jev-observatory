...
const metricValue=value=>/^\d+\/\d+$/.test(String(value))?`<div class="metric-ratio"><strong>${fmt(String(value).split('/')[0])}</strong><span>of ${fmt(String(value).split('/')[1])}</span></div>`:esc(value);
export const stat=(label,value,sub)=>`<div class="stat"><div class="label">${esc(label)}</div><div class="value">${metricValue(value)}</div><div class="sub">${esc(sub)}</div></div>`;
