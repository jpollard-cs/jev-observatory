...
test('six-call simulated advisor completes, accounts once, caches and never persists key',async t=>{const x=setup(t);await x.connect();const p=x.prepare(),a=await x.connection.authorize({planHash:p.manifest.planHash});const started=await x.connection.run({token:a.token,planHash:a.planHash,confirmPaid:true});await assert.rejects(()=>x.connection.run({token:a.token,planHash:a.planHash,confirmPaid:true}));const done=await finish(x.connection,started.id);assert.equal(done.status,'complete',done.error);assert.equal(x.calls(),6);assert.equal(done.report.rows.length,6);assert.equal((await x.connection.status()).account.knownUsageUsd,6*100*42/1e9);assert.equal((await x.connection.status()).providerVerified,true);for(const f of files(x.dir))assert.ok(!fs.readFileSync(f,'utf8').includes(KEY),f);});
test('reconfirming a completed exact advisor plan reuses evidence, not paid calls',async t=>{const x=setup(t);await x.connect();const p=x.prepare();for(let i=0;i<2;i++){const a=await x.connection.authorize({planHash:p.manifest.planHash}),start=await x.connection.run({token:a.token,planHash:a.planHash,confirmPaid:true});assert.equal((await finish(x.connection,start.id)).status,'complete');}assert.equal(x.calls(),6);});
...
test('tagging can prepare with incomplete irrelevant policy/app, ranking cannot',()=>{assert.equal(makeAdvisorPlan({name:''},{},{mode:'tag'}).manifest.status,'prepared_offline');assert.throws(()=>makeAdvisorPlan({name:''},{},{mode:'rank'}));});
...
test('cached completion does not verify a newly entered credential',async t=>{
 ...
 assert.equal(x.calls(),6);assert.equal((await x.connection.status()).providerVerified,false);
});
