...
// Historical corpus gold and its protocol hash continue to use profiles 1.0.
// The prospective native policy contract selects the separately versioned profile.
export const prospectiveProfiles = JSON.parse(fs.readFileSync(new URL('../policies/profiles-v1.1.json', import.meta.url)));
export function policyProfileMetadataForVersion(version) {
  const prospective = version === 'policy-v4';
  return {
    version: prospective ? '1.1' : '1.0',
    sha256: hash(JSON.stringify(prospective ? prospectiveProfiles : profiles)),
    file: prospective ? 'policies/profiles-v1.1.json' : 'policies/profiles.json',
    hashBasis: 'JSON.stringify(parsed policy profiles)',
  };
}
