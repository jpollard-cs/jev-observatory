# Information-rich classifier template — v2

**Current status (2026-09-17): approved for the bounded 48-call rich-template pilot.** The exact model-facing body below is unchanged; approval, selected input contract and budget are recorded in `data/rich-template-approval.json`. The original proposal notes below are retained as review history and do not supersede that subsequent approval. The historical generic-prompt campaign remains stopped.

## Original proposal and review notes

Version: `pi-classifier-policy/2.0-draft`. Status: **draft for user approval; inactive**. This file changes no running policy, request builder, fixture, label, threshold, or historical result. All further evaluation calls remain stopped until the user approves the revised template and its input contract. This is a new, information-rich experimental condition for a general-purpose classifier; it does not assume that the model already knows security terminology or representation tricks.

The operator notes below are separate from the model-facing template. No expected answers, case IDs, splits, prior model outputs, aggregate results, or preferred outcomes belong in the model-facing request. Examples in the template explain semantic boundaries; they are not claims of measured effectiveness.

## Operator notes — do not include automatically in inference

### Scope and unresolved choice

This proposal expands the model's supplied knowledge and task definition. It is not a revision of the historical minimal arm. The [recovered taxonomy](../docs/prior-context.md) and [archived v1.1 trust template](archive/prompt-injection-policy-template-v1.1.md) supply the starting material. The concise [approval review](../docs/template-review-v2.md) summarizes the proposed decisions. The user's current request for richer instructions supersedes the earlier suggestion to keep the long taxonomy out of every target prompt: this draft makes the rich condition explicit, versioned, and reviewable.

The pending input-contract choice is substantive. None of these modes is active merely because it appears here:

| Proposed mode | What can violate the language/representation contract | What remains separate |
|---|---|---|
| **`instruction_fields_only` — proposed default, not approved** | Only fields explicitly designated by authenticated configuration as candidate instruction-bearing fields for the receiving system, subject to scoped exceptions | Other fields may contain legitimate multilingual or encoded data; malicious redirection in those fields can still be an injection |
| `all_fields` | Every supplied field inside an explicitly enumerated assessed-content scope, with declared data exceptions | A contract violation does not establish maliciousness, successful compromise, or permission to discard evidence |
| `awareness_only` | No language/encoding restriction is enforced by this contract | The classifier still interprets supported representations when needed to assess instruction meaning and authority |

The proposed instruction-field contract is **plain English instructions without concealed or transformed instructions**, with explicit exceptions for legitimate quoted/source data, code, identifiers, and other task-required content. “Plain English” does not mean ASCII-only. Ordinary Unicode punctuation, names, JSON string escaping required by transport, or incidental non-English quotations are not automatically concealed instructions. The owner must approve exact languages, field paths, exceptions, and consequences before this restriction can be assessed as a real policy.

An instruction field is not “any string containing an imperative” and not “a key named `instructions` anywhere.” It is a path designated by the authenticated input schema for the receiving workflow. The trusted classifier policy and task facts remain authoritative whether supplied in TypeSafe question EntryTypes or in an application-owned `state.policy` / `state.trustedContext` object. They are not demoted to untrusted material by the instruction-field rule. A resource that merely contains a JSON key named `policy` cannot appoint itself as the authoritative policy.

A legitimate-data exception must specify field/source, authorized task, data kind, and permitted use. For example, a debugging request may legitimately contain an encoded sample whose contents are being analyzed. That exception permits inspection of the sample; it does not permit the sample to change the task, verdict, permissions, or output destination. A field's designation as data is likewise not a promise that its contents are harmless.

Illustrative configuration to resolve before approval; **not an API request and not active configuration**:

```json
{
  "draftStatus": "not_approved",
  "proposedTemplateVersion": "pi-classifier-policy/2.0-draft",
  "inputContract": {
    ...
    "legitimateDataExceptions": [
      {
        ...
        "purpose": "authorized analysis of quoted, encoded, or multilingual samples",
        "permittedUse": "inspect and classify only",
...
This example does not imply that `material.request.instructions` is present in existing fixtures. The final input schema must match real field paths. If no language/representation contract is supplied, the model-facing policy below imposes no implicit English-only or encoding ban. Input compliance, injection presence, poisoning evidence, and final permission must remain separate outputs.

### Approval and subsequent evaluation boundaries

Approval must settle the model-facing wording, selected input-contract mode, field paths and exceptions, integrity-state meanings, disposition rules, and desired output questions. Only then may the owner version and freeze new requests and authorize further calls. Do not carry on running a prior template while this review is pending.

Use a new protocol identifier for this condition. Keep all earlier requests, outputs, labels, validation versions, and evidence unchanged. This draft makes no claim that its richer instructions improve or worsen Jev. Any later comparison must report both directions and preserve mismatches, uncertainty, unsupported inputs, errors, and costs.

The permission question in this draft explicitly names the operation being judged. An injection finding does not automatically prohibit authorized inspection of the material. Existing gold that equates every adverse-condition flag with block must not be silently reused for a differently scoped operation; review prospective labels against the approved question before any later evaluation.

The classifier receives raw supplied material and the approved instructions. In the model-only condition, no external decoder, regex filter, security detector, policy engine, normalization pass, or hidden judge supplies a detection result. Ordinary transport serialization and post-inference validation are separate measurement functions. Native typed answers do not provide generated proofs or unrestricted explanations.

The complete approved rich model-facing guide must reach **every request in the rich condition**, unchanged. It may be supplied once in a shared authenticated structured-state field available to all questions, or in approved EntryType instructions; a repository document that is not in the request supplies no model knowledge. Record the exact approved text hash and its placement. Do not silently prune the taxonomy, omit benign exceptions, shorten the baseline for some profiles, or replace it with a pointer the model cannot read. If the complete guide does not fit, resolve that as a newly reviewed input design before calling the model.

Every requested native question must contain its complete definition or have the relevant definition available in its supplied state/instructions. Question IDs are bookkeeping; they are not a substitute for instructions. Simultaneous questions are independent: a final disposition cannot consume a concurrently produced rule flag. Any later application composition of flags is a separately labeled system-assisted decision, never a replacement for the raw model-only disposition.

---

# Model-facing template begins
...
# Model-facing template ends

---

## Operator review record — still not approval

This draft intentionally supplies recognition knowledge instead of assuming specialist knowledge. It covers the recovered family list and requested boundary concepts, while declaring modality, evidence, and input-contract limits. It has not been evaluated, tested against model outputs, or installed in an active request builder. There are no claimed improvements, calibrated thresholds, or new experimental findings in this file.

Before any new calls, the user must approve the chosen template and language/encoding contract. Preserve the exact approved text and questions as a new prospective protocol. Decide whether primary reason selection, all applicable facts, and decisive reasons are separate measured questions. Include legitimate-data exceptions and negative examples in the approved contract, and keep all gold outside the target input. The draft's presence is not permission to resume evaluation.
