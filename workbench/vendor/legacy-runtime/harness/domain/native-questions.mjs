...
import { policyV4Questions } from './policy-question-contract.mjs';

export const NATIVE_REQUEST_VERSIONS = ['legacy-v1', 'legacy-v2', 'advanced-v3', 'policy-v4'];
export const DEFAULT_NATIVE_REQUEST_VERSION = 'policy-v4';
...
export function buildNativeRequest({
  ...
  const questions =
    version === 'policy-v4'
      ? policyV4Questions(specifications, policy, trustedContext)
      : version === 'advanced-v3'
...
export function requestVersionOf(request) {
  const instructions = request.questions.classification.instructions;
  if (instructions?.contractVersion === 'policy-v4') return 'policy-v4';
  return typeof instructions === 'string' ? 'legacy-v2' : 'advanced-v3';
}
