...
export const SETUP_VERSION='reviewed-setup-options/1';
...
export const SETUP_FIELDS=[
 ...
 {id:'languages',title:'Declared language list',question:'Which of these supported language-list changes is explicitly requested by the application description? Do not infer language permission from names, geography, or the language of this description.',criteria:{english:option('English only is explicitly requested.'),english_spanish:option('English and Spanish, and no other languages, are explicitly requested.'),english_french:option('English and French, and no other languages, are explicitly requested.'),manual_review:option('A different language combination or all human languages is requested. Leave the list unchanged for manual selection; this is not verified model coverage.'),...unknown}},
...
export function setupSummaryFromValidated(report){
 ...
 for(const d of SETUP_FIELDS){const answer=answers[d.id];if(!answer){notes.push({field:d.id,text:'No answer supplied; current value retained.'});continue;}
  ...
  if(v==='insufficient_evidence'||v==='manual_review'||v==='other_or_custom'){notes.push({field:d.id,title:d.title,text:v==='manual_review'?'Select the requested languages manually. The suggestion library does not cover that combination.':v==='other_or_custom'?'Keep your custom task. The three built-in dossiers do not validate this application automatically.':'More context is needed; no change is proposed.',signal});continue;}
  ...
  else if(d.id==='languages'){const values={english:['en'],english_spanish:['en','es'],english_french:['en','fr']};if(values[v])changes=[{target:'policy',path:['languages','mode'],before:p.languages.mode,after:'allowlist'},{target:'policy',path:['languages','allowed'],before:p.languages.allowed,after:values[v]}];reason='The description explicitly identifies this language list.';consequence='Changes permitted languages. Permission is not demonstrated language coverage; names and ordinary Unicode remain distinct.';}
...
export function applySetupSummary(policy,application,summary,selected){
 ...
 return {before:{policy:p,application:a},after:next,selected:[...selected],adviceReportHash:summary.reportHash,afterKey:applicationKey(next.policy,next.application),kind:'explicit_setup_review'};
}
