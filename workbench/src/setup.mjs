...
import {reviewLanguageChoice} from './setup-language.mjs';
...
export const SETUP_VERSION='reviewed-setup-options/4';
// Only options offered for the target enter its bounded advisor vocabulary.
export function setupLanguageLists(model,registry){
 const view=languageOptionsForModel(model,registry);
 assert(view,'Language capabilities are not registered for this model');
 const lists=Object.fromEntries(view.options.map(({code,label})=>[label.toLowerCase().replaceAll(' ','_'),[code]]));
 for(const [id,codes] of Object.entries({english_spanish:['en','es'],english_french:['en','fr']}))if(codes.every(code=>view.options.some(x=>x.code===code)))lists[id]=codes;
 return lists;
}
export const SETUP_LANGUAGE_LISTS=setupLanguageLists('jev-1.13.0');
...
const outputLanguageOnly=option('Only generated replies, answers or policy wording have a language requirement; no input-language rule is specified. This cannot change admitted input languages or input scope.');
...
export function setupFieldsForModel(model,registry){
 const languages=languageOptionsForModel(model,registry),lists=setupLanguageLists(model,registry);
 const names=Object.fromEntries(languages.options.map(x=>[x.code,x.label]));
 return [
 ...
 {id:'language_scope',title:'Language-rule scope',question:'Classify INPUT-language scope only. Does an explicit INPUT restriction cover all source content, or only actual controlling instructions? A request to write replies, reviews or answers in a language controls OUTPUT, not the language of controlling input instructions. Choose output_language_only when the requirement concerns only generated output. Do not infer that an English description itself imposes English-only on customer data. Choose not_specified when no input-language restriction or scope is stated; reserve insufficient_evidence for conflicting or ambiguous requirements.',criteria:{output_language_only:outputLanguageOnly,not_specified:option('No input-language rule or scope is stated. Keep the current scope as a disclosed default, not a model recommendation.'),natural_language_content:option('An explicit language limit applies to all incoming natural-language content.'),controlling_instructions:option('The application explicitly allows descriptive/quoted data in other languages while limiting the language of controlling instructions.'),...unknown}},
 {id:'languages',title:'Declared language list',question:'Which permitted input languages are explicitly requested by application.description? A single-language requirement replaces the current list: Spanish-only means Spanish without English. The description language and currentDraft default do not grant language permission. If the requirement only concerns the language of generated replies, review responses or policy wording, choose output_language_only even if the description explicitly says input languages are unaffected; this field controls admitted input content. Choose not_specified when no input-language requirement is stated. Reserve insufficient_evidence for conflicting or ambiguous language requirements.',criteria:{output_language_only:outputLanguageOnly,...Object.fromEntries(Object.entries(lists).map(([id,codes])=>[id,option(codes.map(code=>names[code]).join(' and ')+' and no other input languages are explicitly permitted.')])),not_specified:option('No admitted-input language requirement is stated. Retain the exact current language setting as a disclosed default; do not infer English-only from the description language or silently permit all languages.'),any_language:option('All human languages are explicitly permitted as input; this is permission, not demonstrated model coverage.'),manual_review:option('A different combination, regional variant, or unsupported input-language requirement needs manual review. Do not substitute an English-containing list.'),...unknown,keep_current:option('The description explicitly supports the current input-language list and requests no different language rule. A current default is not evidence of the requested permissions.')}},
...
export const SETUP_FIELDS=setupFieldsForModel('jev-1.13.0');
...
export function setupSummaryFromValidated(report){
 ...
 for(const d of setupFieldsForModel(p.model)){const answer=answers[d.id];if(!answer){notes.push({field:d.id,text:'No answer supplied; current value retained.'});continue;}
  const signal=setupSignal(answer),v=answer.choice;let changes=[],reason='',consequence='';
  if(['languages','language_scope'].includes(d.id)&&['languages','language_scope'].some(id=>answers[id]?.choice==='output_language_only')){
   if(d.id==='languages')notes.push({field:'generated_reply_language',title:'Reply-language requirement',disposition:'review',text:'The advice identifies a reply-language requirement. This compiler controls admitted inputs; it does not enforce the language of generated replies. Input languages and scope remain unchanged. Configure output-language behavior in your application separately.',signals:{languages:setupSignal(answers.languages),language_scope:answers.language_scope?setupSignal(answers.language_scope):null}});continue;
  }
  if(v==='not_specified'&&['languages','language_scope'].includes(d.id)){
   notes.push({field:d.id,title:d.title,disposition:signal.uncertain?'review':'default_retained',text:signal.uncertain?'Jev is unsure whether a language requirement was specified. Review the current setting.':d.id==='languages'?'No input-language requirement was identified. The current language setting is retained; choose a different one below if needed.':'No language scope was specified. The current scope is retained.',signal});continue;
  }
...
export function applySetupSummary(policy,application,summary,selected,languageChoice=null){
 ...
 assert(Array.isArray(selected)&&(selected.length||languageChoice!==null)&&new Set(selected).size===selected.length,'Select at least one distinct proposed change');
 assert(languageChoice===null||!selected.includes('languages'),'Choose either the model language proposal or your own language choice');
 ...
 const languageDecision=languageChoice===null?null:reviewLanguageChoice(next.policy,languageChoice);
 if(languageDecision)next.policy.languages=languageDecision.after;
 validatePolicy(next.policy);validateApplication(next.application);
 return {before:{policy:p,application:a},after:next,selected:[...selected],ownerLanguageDecision:languageDecision,unresolvedNotes:clone(summary.notes.filter(n=>!languageDecision||n.field!=='languages')),unappliedSuggestions:clone(summary.suggestions.filter(s=>!selected.includes(s.id)&&(!languageDecision||s.id!=='languages'))),adviceReportHash:summary.reportHash,afterKey:applicationKey(next.policy,next.application),kind:'explicit_setup_review'};
}
