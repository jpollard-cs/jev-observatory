...
export function mandatoryUnits(p,a){
 ...
 if(a.surfaces.includes('conversation_history'))ids.push('crescendo-gradual-concealment-full');
 if(a.surfaces.includes('tool_results')||a.capabilities.includes('external_actions'))ids.push('pf-tool-output-spoof');
 if(a.surfaces.includes('persistent_memory')||a.capabilities.includes('memory_writes'))ids.push('pf-memory-poisoning');
 if(a.capabilities.includes('sensitive_disclosure'))ids.push('pf-data-exfil');
 if(a.capabilities.includes('judging'))ids.push('pf-judge-injection');
 if(a.capabilities.includes('moderation'))ids.push('pf-moderation-injection');
...
function gapsFor(p,a){const gaps=[];for(const l of p.languages.mode==='allowlist'?p.languages.allowed:[])if(!['en','es','fr','de'].includes(l))gaps.push({kind:'unrepresented_language',id:l,blocking:false,message:`No authored language probes for ${l}; enabling it is not validated by this catalog.`});
 ...
 for(const f of new Set(critical))if(['tools_and_disclosure','persistent_state'].includes(f)&&UNITS.some(u=>u.facets.includes(f)))gaps.push({kind:'unsupported_critical_boundary',id:f,blocking:true,message:`Classifier probes recognize ${f} injection attempts; real tool execution, disclosure and persistent-memory enforcement are not tested.`});
 for(const c of a.capabilities.filter(c=>['judging','moderation'].includes(c)))gaps.push({kind:'unsupported_evaluation_adapter',id:c,blocking:true,message:`This policy suite does not evaluate ${c}. Its original suite runs separately and does not validate this policy.`});return gaps;
}
export function assistedOptions(o={}){exactKeys(o,['tier','maxUsd','maxInputTokens','layouts','seed','includeContext','repeat','explorationShare','coverageScope','attackPacks'],'Assisted options');const x={tier:'budget',maxUsd:.15,maxInputTokens:3000000,layouts:['question','criteria'],seed:'coverage-advisor-1',includeContext:true,repeat:1,explorationShare:.15,coverageScope:'full_declared_scope',attackPacks:ATTACK_PACKS.map(p=>p.id),...o};
 assert(Array.isArray(x.attackPacks)&&new Set(x.attackPacks).size===x.attackPacks.length&&x.attackPacks.every(id=>ATTACK_PACKS.some(p=>p.id===id)),'Unknown attack pack');assert(['full_declared_scope','available_policy_tests'].includes(x.coverageScope),'Invalid evaluation scope');assert(['budget','bronze','silver','gold'].includes(x.tier),'Invalid assisted tier');validateBudget(x.maxUsd);x.maxInputTokens=validateTokenLimit(x.maxInputTokens);assert(Array.isArray(x.layouts)&&x.layouts.length>0&&x.layouts.length<=2&&new Set(x.layouts).size===x.layouts.length&&x.layouts.every(s=>['question','criteria'].includes(s)),'Invalid layouts');assert([1,2,3].includes(x.repeat),'Invalid repeat');assert(typeof x.seed==='string'&&x.seed.length>0&&x.seed.length<200,'Invalid seed');assert(typeof x.includeContext==='boolean','Invalid context option');assert(Number.isFinite(x.explorationShare)&&x.explorationShare>=.05&&x.explorationShare<=.5,'Independent exploration share must be 5–50%');return x;}
export function makeAssistedPlan(policy,application,options={},report=null){
 ...
 const jobMap=new Map();for(const c of CATALOG){if((!o.includeContext&&c.kind==='task-grounded-context')||(c.pack&&!o.attackPacks.includes(c.pack)))continue;for(let repeat=1;repeat<=o.repeat;repeat++)for(const layout of o.layouts){const r=compileCase(p,c,layout),id=c.id+'__'+layout+'__r'+repeat;jobMap.set(id,{id,caseId:c.id,group:c.group,layout,repeat,sourceVersion:c.sourceVersion,kind:c.kind,evaluation:clone(c.evaluation??null),requestHash:r.requestHash,wireBytes:r.wireBytes,estimatedInputTokens:r.estimatedInputTokens,reservationInputTokens:r.reservationInputTokens,body:r.body,receipt:r.receipt,expected:expectedFor(p,c)});}}
 const units=UNITS.filter(u=>(o.includeContext||!u.domain)&&(!u.pack||o.attackPacks.includes(u.pack))).map(u=>({...clone(u),jobs:[...new Set(u.caseIds.flatMap(id=>Array.from({length:o.repeat},(_,i)=>o.layouts.map(l=>id+'__'+l+'__r'+(i+1))).flat()))].map(id=>{const j=jobMap.get(id);assert(j,'Missing unit job');return {id,nano:j.reservationInputTokens*42,tokens:j.reservationInputTokens};})}));
 const mandatory=mandatoryUnits(p,a),missingByFilter=mandatory.filter(id=>!units.some(u=>u.id===id));
 const gaps=gapsFor(p,a);for(const id of missingByFilter)gaps.push({kind:'critical_context_excluded',id,blocking:true,message:'A required test group is excluded by the selected packs or context settings: '+id});
 ...
 // A scoped run defers absent evaluators, never an available mandatory check or a budget limit.
 const deferredGapIds=o.coverageScope==='available_policy_tests'?gaps.filter(g=>['unsupported_critical_boundary','unsupported_evaluation_adapter'].includes(g.kind)).map(g=>g.kind+':'+g.id):[];
 const evaluationScope={mode:o.coverageScope,deferredGapIds};
 const state=gaps.some(g=>g.blocking&&!deferredGapIds.includes(g.kind+':'+g.id))?'insufficient_coverage':allocation.state;
