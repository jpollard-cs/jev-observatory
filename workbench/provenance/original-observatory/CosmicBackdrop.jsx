...
export function CosmicBackdrop({ motion }) {
  const field = useRef(null),
    arms = useRef(null),
    echo = useRef(null),
    host = useRef(null);
  useEffect(() => {
    paint(field.current, false);
    paint(arms.current, true);
    const ctx = echo.current.getContext('2d');
    echo.current.width = 1024;
    echo.current.height = 1024;
    ctx?.drawImage(arms.current, 0, 0);
    const visibility = () => {
      if (host.current) host.current.dataset.hidden = String(document.hidden);
    };
    visibility();
  ...
  return (
    ...
    >
      <canvas ref={field} className="cosmic-stars" />
      <div className="cosmic-arm cosmic-arm-a">
        <canvas ref={arms} />
      </div>
      <div className="cosmic-arm cosmic-arm-b">
        <canvas ref={echo} />
      </div>
