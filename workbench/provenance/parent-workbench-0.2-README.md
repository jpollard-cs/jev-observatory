...
Extract `jev-policy-workbench-0.2.zip` onto Desktop. Node.js 22 or later is required; no package installation is necessary.

```bash
bash "$HOME/Desktop/jev-policy-workbench-0.2/run.sh"
```

Open the printed address, normally **http://127.0.0.1:8788**. The earlier 0.1 release used port 8787 and may remain installed. There is no installer. Do not copy this folder over a running experiment or move `.env`.
...
- **No new live measurements.** Only software tests and synthetic-transport simulations were performed for 0.2. It has not been shown that Jev-assisted selection outperforms deterministic selection.
