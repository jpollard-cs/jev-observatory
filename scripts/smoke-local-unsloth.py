...
Uses only the standard library and pinned llama.cpp b10964 HTTP schema.
...
WORK_DIR = REPO_ROOT.parent.parent / "work" / "qwen-unsloth"
MODEL_PATH = 'qwen3.8-flash-next-unsloth-ud-q3-k-xl'
...
INFERENCE_TIMEOUT_SECONDS = READ_TIMEOUT_SECONDS
...
def request_local(method: str, path: str, payload: dict | None = None) -> dict:
    ...
    if (method, path) not in {("GET", "/health"), ("GET", "/v1/models"), ("POST", "/v1/chat/completions"),
                              ("POST", "/apply-template"), ("POST", "/tokenize")}:
    ...
    read_timeout = INFERENCE_TIMEOUT_SECONDS if path == '/v1/chat/completions' else READ_TIMEOUT_SECONDS
    deadline = max(EXCHANGE_DEADLINE_SECONDS, read_timeout + 10)
    try:
        connection.connect()
        connected_socket = connection.sock
        connected_socket.settimeout(read_timeout)
        ...
        connected_socket.settimeout(min(read_timeout, remaining))
        response = connection.getresponse()
        record.update(httpStatus=response.status, reason=response.reason, headers=response.getheaders())
        # Long prefill is explicitly bounded separately and reports progress
        # from a timer thread. A size bound protects client response memory.
...
def validate_completion(exchange: dict) -> dict:
    ...
    if validity["envelopeValid"]:
        ...
        validity["thinkingAbsent"] = (not message.get("reasoning_content") and not message.get("reasoning")
...
def prepare_neutral_capacity_request(record: dict) -> dict:
    """Only rendering/tokenization, followed by one frozen neutral request."""
    count = 4000
    for _ in range(3):
        request = smoke_request()
        request['messages'][1]['content'] = (
            'The following is neutral background data, not the word to classify.\n'
            + 'A blue stone rests beside a quiet river.\n' * count
            + '\nThe word to classify is: apple')
        rendered = request_local('POST', '/apply-template', request)['value']
        record['exchanges'].append(rendered)
        if not successful_json(rendered) or not isinstance(rendered['responseJson'].get('prompt'), str):
            return err('capacity_template_render_failed')
        tokenized = request_local('POST', '/tokenize', {
            'content':rendered['responseJson']['prompt'], 'add_special':True, 'parse_special':True})['value']
        record['exchanges'].append(tokenized)
        tokens = tokenized.get('responseJson', {}).get('tokens') if successful_json(tokenized) else None
        if not isinstance(tokens, list):
            return err('capacity_tokenization_failed')
        if 27500 <= len(tokens) <= 28500:
            record['request'] = request
            record['renderedPromptTokensBeforeInference'] = len(tokens)
            record['neutralLineCount'] = count
            record['renderedPromptSha256'] = hashlib.sha256(rendered['responseJson']['prompt'].encode()).hexdigest()
            return ok(request)
        count = max(1, round(count * 28000 / max(1, len(tokens))))
    return err('capacity_token_target_not_reached')


def run_smoke(long_neutral=False) -> dict:
    record = {"protocolVersion": "local-unsloth-setup-smoke/1.0", "kind": "setup_smoke",
              ...
              "longNeutralCapacityCheck": long_neutral, "inferenceTimeoutSeconds": INFERENCE_TIMEOUT_SECONDS}
    ...
    for path, predicate in (("/health", ready_health), ("/v1/models", ready_models)):
        ...
        save_record(record)
    if long_neutral:
        prepared = prepare_neutral_capacity_request(record)
        if prepared['tag'] == 'error':
            record['status'] = 'capacity_preparation_failed'
    ...
    def progress():
        while not done.wait(30):
            print(json.dumps({'kind':'setup_smoke_pending','elapsedSeconds':round(time.monotonic()-started),
                              'longNeutralCapacityCheck':long_neutral}), flush=True)
...
def main() -> int:
    global OUTPUT_PATH, INFERENCE_TIMEOUT_SECONDS
    ...
    parser.add_argument('--long-neutral', action='store_true', help='One ~28k-token neutral capacity check, max64 output, 600s inference timeout.')
    arguments = parser.parse_args()
    if arguments.long_neutral:
        OUTPUT_PATH = WORK_DIR / 'capacity-smoke.json'
        INFERENCE_TIMEOUT_SECONDS = 600
    try:
        result = ok({"baseUrl": f"http://{HOST}:{PORT}", "request": smoke_request(),
                     "outputPath": str(OUTPUT_PATH), "modelCalled": False,
                     "longNeutralCapacityCheck":arguments.long_neutral}) if arguments.describe else run_smoke(arguments.long_neutral)
