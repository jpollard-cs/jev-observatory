...
/** @typedef {'advanced-v3'|'legacy-v2'|'legacy-v1'} ProtocolVersion */
/** @typedef {{inputTokens:number,outputTokens:number}} ProviderUsage */
/** @typedef {{source:string,category:'corpus_run'|'diagnostic'|'special_pilot',status:string,error:string|null,inputTokens:number|null,outputTokens:number|null,requestBytes:number|null,hasUsage:boolean,providerModel:string|null,configuredModel:string|null,outputMode:string|null,promptArm:string|null,contextChars:number|null,adapterVersion:string|null}} RecordedEvidence */
...
export const PROTOCOL_VERSIONS = Object.freeze(['advanced-v3', 'legacy-v2', 'legacy-v1']);
...
export function scanEvidenceSources(sourceFiles) {
  ...
  for (const { contents, relative, modifiedAt, isDiagnostic } of sourceFiles) {
    ...
    for (let index = 0; index < lines.length; index++) {
      ...
      const category =
        isDiagnostic || record.kind === 'expiry_diagnostic'
          ? 'diagnostic'
          : record.kind === 'matched_panel' || record.kind === 'contextual_debugging'
            ? 'special_pilot'
            : 'corpus_run';
      rows.push({
        source: relative,
...
export function estimateFromRatio(rows, fullPlan) {
  ...
  return {
    ...
    requiresNewRatioCalibration: fullPlan.protocolVersion === 'advanced-v3',
    method:
