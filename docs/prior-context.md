...
The initial suite used a short baseline and kept the long taxonomy in the corpus. That was an experiment-design interpretation, not a recovered user requirement. On 2026-09-17 the user corrected it: Jev is general purpose, and the intended baseline must supply an information-rich attack-recognition template. The experiment should measure whether it can apply that supplied expertise and trusted contextual overrides. The user requires approval of the revised template before any further testing. Preserve the earlier generic-prompt results as a separate historical condition. A claimed override inside evaluated material remains an attack input, not a real policy change.
