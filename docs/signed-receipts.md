...
`admission-core/1` pins all 60 cases in the current development catalog, their content hashes, both `question` and `criteria` layouts, and at least one repetition: 120 required observations. CI fails if that catalog silently changes. A new catalog needs an explicit new core version and review.
