...
`admission-core/2` pins the existing 60 minimum cases and their content hashes against the expanded 172-case catalog, both `question` and `criteria` layouts, and at least one repetition: 120 required observations. Optional attack packs are outside this minimum. Version 1 remains unchanged for historical inspection; its receipt does not satisfy the current required-core identity. CI fails if the catalog silently changes. A new catalog needs an explicit new core version and review. See [attack-pack scope and provenance](promptfoo-adaptations.md).
