...
`POST /api/execution/runs/:id/recover` checks the owner's existing saved responses, including original signatures for signed runs, and settles a complete batch exactly once. It makes no provider requests. Missing, altered or incorrectly bound evidence leaves the existing claim and hold untouched. Older unsigned runs remain unsigned. Unknown billing is never silently cleared. The owner may explicitly keep each unresolved reservation and then authorize a separate run using the remaining allowance. Neither UI refresh nor recovery invents a provider charge or removes an unresolved hold.

## Continuing with unresolved charges

A ready run shows the unresolved reservations and offers **Keep the amount reserved and continue** before ordinary key/spending authorization. Acknowledging alone sends no request. The start transaction matches each reviewed run ID, integer reservation and update timestamp, saves those snapshots on the new run, and checks the entire account ledger including all older holds. A changed or newly unresolved reservation requires another review. An active run always blocks a different start; overlapping starts cannot both reserve funds.

This is authorization metadata, not a billing settlement or model evidence. Old rows, observations and completion signatures remain unchanged. Late saved responses can still settle the old reservation exactly once without disturbing the new run. Existing rows migrate with an empty acknowledgement list; default authorization continues to block unresolved holds.
