...
The API key is held by the tab-local credential session for up to 30 minutes, sent over HTTPS with each authorized step, used transiently by the server and then discarded server-side. It is not in localStorage, persistent server state, URLs, exports or application logs. Closing/reloading the page, disconnecting or expiry forgets it. Completion and stop retain it for subsequent explicitly reviewed requests during the session. Expiry also prevents later dispatch. See `inline-assisted-workflow.md` for the updated interface and credential lifecycle. There is no operator-owned shared Jev credential and no paid connection probe. The operator necessarily handles the key transiently; browser encryption cannot protect it against malicious code already executing in the page. Runtime request-body logging must remain disabled.

D1 stores the account allowance and run state. R2 stores immutable request manifests, individual requests and observations under private owner-scoped keys. Account creation is insert-once: reloading, another key or another tab cannot reset its prior usage or limit. An atomic database transition reserves the entire plan and allows one active run per account. A second atomic transition claims a batch of up to sixteen requests before network I/O. The browser defaults to eight at a time and also offers serial execution. All responses in a batch are saved under their original manifest indices before a single atomic settlement. Per-response provenance records the batch start and size. A failed response stops the next batch; already dispatched siblings may finish and are still accounted for. An evidence-storage failure leaves the entire batch claimed and held; it cannot be replayed. Success settles usage; an uncertain charge retains its reservation. Stopped runs do not lock execution; their reservations remain deducted from the available allowance. Cancel prevents later claims, but an already claimed request may finish and remain billable. No SDK or transport retries are enabled. HTTP errors and provider bodies are not echoed to users or logs.
...
Initial hosted limits are 480 requests / 24 MiB request material per plan, 100 retained plans per account, one active run, up to three concurrent requests within that run, and a $3 account authorization. Larger suites and all original test definitions remain available to export/run locally. Batches are not silently truncated. Completed reports remain private; opening the community library does not publish them. Browsing/offline planning can be anonymous once the site is public; hosted execution currently uses Sites' ChatGPT identity. A non-ChatGPT execution login is not implemented.
...
`POST /api/execution/runs/:id/recover` checks the owner's existing saved responses, including original signatures for signed runs, and settles a complete batch exactly once. It makes no provider requests. Missing, altered or incorrectly bound evidence leaves the existing claim and hold untouched. Older unsigned runs remain unsigned. Unknown billing is never silently cleared. The owner can authorize a separate run using the remaining allowance; all unresolved reservations stay deducted automatically. Neither UI refresh nor recovery invents a provider charge or removes an unresolved hold.

## Continuing with unresolved charges

Stopped runs cannot send further requests. Their held amount is a conservative local reservation for uncertain provider charges, not an active execution lock. A new request proceeds through the ordinary key/spending review if it fits the remaining allowance. No extra acknowledgement or second cancellation is required.

The atomic start transition blocks only another `running` record and checks the full account total of known charges plus every held reservation. It records snapshots of existing stopped holds in `retained_holds_json` directly from the same owner's database rows. Client-provided hold lists cannot release money, bypass accounting, or change this audit metadata. These snapshots describe reservations at authorization, not provider-signed billing evidence.

The interface separates active runs, unstarted plans, stopped runs with reserved charges and completed history. A genuinely active blocker has an inline Stop action. Late saved responses can settle an old claim exactly once without disturbing another run; old evidence and completion signatures remain unchanged. The previous per-request hold acknowledgement has been removed: it conflated uncertain billing with active execution and made already-stopped runs appear uncancellable.
