...
D1 stores the account allowance and run state. R2 stores immutable request manifests, individual requests and observations under private owner-scoped keys. Account creation is insert-once: reloading, another key or another tab cannot reset its prior usage or limit. An atomic database transition reserves the entire plan and allows one active run per account. A second atomic transition claims a batch of up to three requests before network I/O. The browser defaults to three at a time and also offers serial execution. All responses in a batch are saved under their original manifest indices before a single atomic settlement. Per-response provenance records the batch start and size. A failed response stops the next batch; already dispatched siblings may finish and are still accounted for. An evidence-storage failure leaves the entire batch claimed and held; it cannot be replayed. Success settles usage; an uncertain charge retains its reservation and blocks later runs until reconciled. Cancel prevents later claims, but an already claimed request may finish and remain billable. No SDK or transport retries are enabled. HTTP errors and provider bodies are not echoed to users or logs.
